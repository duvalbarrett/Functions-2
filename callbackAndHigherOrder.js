...
const last = (array, callback) => callback(array[6])
...
const uniq = (array, callback) => {
  for (let i = 0; i < array.length; i++){
    for (let n = n + 1; n < array.length; n++){
      if(array[i] === array[n]){
        callback(array.splice(i,1))
      } n--
    }
  }
  }
...
uniq(names, uniqArr => callback(`The new names array with all the duplicate items removed is ${uniqArr}`))
...
const realStuff = (item, index) => console.log(`the item at index ${index} is ${item}`) 
each(names, realStuff)
...
const getUserById = (users, identity, cb => {
  for(let i = 0; i < array.length; i++){
    if (users[i].id === identity){
      cb(users[i])
    }
  }
})
...
const addingFactory = num1 => (function2) => num1 + num2

console.log(addingFactory(5))

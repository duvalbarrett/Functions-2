...
const totalPopulation = populations.reduce(function(runningTotal,curElement,curIndex,wholeArray){
    return console.log(runningTotal + curElement)
});
...
const orderTotals = orders.map(function(element){
  return acc + (element.price * element.tax)
})
...
const justPrices = purchases.map(function(element){
  return element.price
});
const bobsTotal = purchases.reduce(function(acc, curr){
  return acc + curr
});
